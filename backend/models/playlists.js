...
class Playlist {
  ...
  static async addTracks(playlistID, track) {
    const newTrack = db.query(`INSERT INTO tracks (id, title, image_url,audio, playlist_id) VALUES ($1, $2, $3, $4, $5) RETURNING title, image_url, audio`, [track.id, track.name, track.image, track.audio, playlistID])  
    const result = await db.query(`INSERT INTO playlists_tracks (track_id, playlist_id) VALUES ($1, $2)`, [track.id, playlistID]);
